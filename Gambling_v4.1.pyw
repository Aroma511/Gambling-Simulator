...
def bilder():
    ...
    _75_pos_1000 = tk.PhotoImage(file="photos/coin75-pos1000.png")
    _100_pos_1000 = tk.PhotoImage(file="photos/coin100-pos1000.png")
    _200_pos_1000 = tk.PhotoImage(file="photos/coin200-pos1000.png")
    _500_pos_1000 = tk.PhotoImage(file="photos/coin500-pos1000.png")
    _1000_pos_1000 = tk.PhotoImage(file="photos/coin1000-pos1000.png")
    menü_links = tk.PhotoImage(file="photos/menue-links.png")
    zurück_links = tk.PhotoImage(file="photos/zurueck-links.png")
    menü_rechts = tk.PhotoImage(file="photos/menue-rechts.png")
    zurück_rechts = tk.PhotoImage(file="photos/zurueck-rechts.png")
...
def check_for_1000():
    if coins > 10000:
        tausend_button.place(x=374, y=556, width=90, height=90)
        tausend_button.config(image=_1000_pos_1000)

        fünf_hundert_button.place(x=340, y=645, width=90, height=90)
        fünf_hundert_button.config(image=_500_pos_1000)

        zwei_hundert_button.place(x=234, y=686, width=90, height=90)
        zwei_hundert_button.config(image=_200_pos_1000)

        hundert_button.place(x=131, y=647, width=90, height=90)
        hundert_button.config(image=_100_pos_1000)

        fünfundsiebzig_button.place(x=96, y=554, width=90, height=90)
...
def öffne_zweites_fenster():
    global aktualisiere_coins, url, coins, benutzer_abrufen, coins_k, Benutzername_anzeige, entry_name, entry_passwort, button_speichern, button_abrufen, label_ergebnis, Passwort_anzeige, entry_passwort, speichere_zeitstand, time_running
    ...
    def lade_daten():
        try:
            url = "http://infoneumayer.bplaced.net/dateien_sim/api.php"
    ...
    def speichere_daten(alle_daten):
        try:
            url = "http://infoneumayer.bplaced.net/dateien_sim/api.php"
            response = requests.post(url, json=alle_daten)
            print("Statuscode:", response.status_code)
            print("Antworttext:", response.text)
            response.raise_for_status()
            print("Daten erfolgreich online gespeichert!")
        except Exception as e:
    ...
    Benutzername_anzeige = tk.Label(root, text="Benutzername:", bg="#E8D6AB")
    Benutzername_anzeige.place(x=230, y=320)

    entry_name = tk.Entry(root)
    entry_name.place(x=210, y=350)

    # Eingabefeld für das Passwort
    Passwort_anzeige = tk.Label(root, text="Passwort:", bg="#E8D6AB")
    Passwort_anzeige.place(x=243, y=380)

    entry_passwort = tk.Entry(root, show="*")  # Verstecke Eingabe mit Sternchen
    entry_passwort.place(x=210, y=410)

    # Button zum Speichern des Benutzers
    button_speichern = tk.Button(root, text="Benutzer speichern", command=benutzer_speichern)
    button_speichern.place(x=160, y=440)

    # Button zum Abrufen des Benutzers
    button_abrufen = tk.Button(root, text="Benutzer abrufen", command=benutzer_abrufen)
    button_abrufen.place(x=280, y=440)

    # Ergebnislabel für die Ausgabe
    label_ergebnis = tk.Label(root, text="", bg="#E8D6AB")
    label_ergebnis.place(x=200, y=470)
...
def benutzer_off():
    if clicking:
        ...
        click_sound_buttons()
        menü_button.place(x=42, y=0, width=80, height=100)
        benutzer_button.place(x=428, y=0, width=80, height=100)
...
def toggle_benutzer():
    ...
    if benutzer_show:
        if clicking:
            ...
            benutzer_off()
            benutzer_button.config(image=menü_rechts)
    else:
        if clicking:
            pass
        else:
            benutzer_on()
            benutzer_button.config(image=zurück_rechts)
...
def menü_on():
    if clicking:
       pass
        ...
        rollen_frame.place(x=0, y=0, width=550, height=800)
        menü_button.lift()
        music_toggle_button.place(x=230, y=330, width=30, height=30)
        music_toggle_button.lift()
        clicks_label.place(x=240, y=370)
        clicks_label.lift()
        benutzername_frame.place_forget()
        sound_toggle_button.place(x=270, y=330, width=30, height=30)
        highscore_label.place(x=230, y=400)
        time_label.place(x=222, y=430)
        time_label.config(text=f"Zeit: {days:02}:{hours:02}:{minutes:02}:{seconds:02}")
...
def toggle_menü():
    ...
    if menü_show:
        if clicking:
            ...
            menü_off()
            menü_button.config(image=menü_links)
    else:
        if clicking:
            pass
        else:
            menü_on()
            menü_button.config(image=zurück_links)
...
#----------------------------------------->
#Menü&Benutzer
menü_button = tk.Button(root, image=menü_links, command=toggle_menü, font=("Arial", 17), bg="#000000", relief="flat")
menü_button.place(x=42, y=0, width=80, height=100)

benutzer_button = tk.Button(root, image=menü_rechts, command=toggle_benutzer, font=("Arial", 17), bg="#000000", width=50, height=50, relief="flat")
benutzer_button.place(x=428, y=0, width=80, height=100)
#----------------------------------------->
...
#----------------------------------------->
#Clicks Label----------------------------------------->
clicks_label = tk.Label(root, text=f"Clicks: {clicks_angezeigt}", bg="#E8D6AB")
clicks_label.place(x=200, y=340)
#----------------------------------------->
#Highscore Label----------------------------------------->
highscore_label = tk.Label(root, text="Highscore: 0", bg="#E8D6AB")
highscore_label.place(x=200, y=370)

# Label zur Anzeige der Zeit
time_label = tk.Label(root, text="Zeit: 00:00:00:00", bg="#E8D6AB")
time_label.place()
#----------------------------------------->
#start variable
